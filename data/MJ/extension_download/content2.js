...
function extractLinks() {
    ...
    setTimeout(() => {
        ...
        anchors.forEach(anchor => {
            ...
            const formattedStyle = style.replace(/&quot;/g, '"');

            // 定义正则表达式匹配 2x 的 URL
            const regex = /url\("([^"]*?)"\)\s*2x/;
            const match = formattedStyle.match(regex);
            let url2x = 'N/A'; 
            if (match && match[1]) {
                url2x = match[1];
                console.log('2x的图片URL:', url2x);
            } 
            if (href) {
...
function startClicking() {
  ...
  const interval = setInterval(() => {
    ...
    clickCount++;
    console.log(`点击第${clickCount}次，提取了 ${links.length/2} 个链接`);
    // 点击按钮并提取链接
